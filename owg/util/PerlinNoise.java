 ...
 * <p>
 *
 * The result is a continuous function that interpolates a smooth path along a series random points. The function is consitent, so given the same parameters, it will always return
 * the same value. The smoothing function is based on the Improving Noise paper presented at Siggraph 2002.
 * <p>
...
public class PerlinNoise
{
    ...
    {
        this.p_imp = new int[this.DEFAULT_SAMPLE_SIZE << 1];

        int i, j, k;
        this.rand = new Random(seed);

        // Calculate the table of psuedo-random coefficients.
        for (i = 0; i < this.DEFAULT_SAMPLE_SIZE; i++)
        {
            this.p_imp[i] = i;
        }
        ...
        {
            k = this.p_imp[i];
            j = (int) (this.rand.nextLong() & this.DEFAULT_SAMPLE_SIZE);
    ...
    public double improvedNoise(double x, double y, double z)
    {
        ...
        double c2 = this.grad(this.p_imp[ba], xo - 1, yo, zo);
        double c3 = this.grad(this.p_imp[ab], xo, yo - 1, zo);
        double c4 = this.grad(this.p_imp[bb], xo - 1, yo - 1, zo);
        double c5 = this.grad(this.p_imp[aa + 1], xo, yo, zo - 1);
        double c6 = this.grad(this.p_imp[ba + 1], xo - 1, yo, zo - 1);
        double c7 = this.grad(this.p_imp[ab + 1], xo, yo - 1, zo - 1);
        double c8 = this.grad(this.p_imp[bb + 1], xo - 1, yo - 1, zo - 1);

        return this.lerp(w, this.lerp(v, this.lerp(u, c1, c2), this.lerp(u, c3, c4)),
                this.lerp(v, this.lerp(u, c5, c6), this.lerp(u, c7, c8)));
    }
    ...
    {
        float t = x + this.N;
        int bx0 = ((int) t) & this.BM;
        int bx1 = (bx0 + 1) & this.BM;
        float rx0 = t - (int) t;
        float rx1 = rx0 - 1;
    ...
    public float noise2(float x, float y)
    {
        ...
        t = y + this.N;
        int by0 = ((int) t) & this.BM;
        int by1 = (by0 + 1) & this.BM;
        float ry0 = t - (int) t;
        float ry1 = ry0 - 1;
    ...
    public float noise3(float x, float y, float z)
    {
        ...
        t = z + (float) this.N;
        int bz0 = ((int) t) & this.BM;
        int bz1 = (bz0 + 1) & this.BM;
    ...
    /**
     * Create a turbulent noise output based on the core noise function. This uses the noise as a base function and is suitable for creating clouds, marble and explosion effects.
     * For example, a typical marble effect would set the colour to be:
     *
     * <pre>
    ...
    {
        return (this.noise2(x, y) * (w - x) * (h - y) +
                this.noise2(x - w, y) * x * (h - y) +
                this.noise2(x, y - h) * (w - x) * y +
                this.noise2(x - w, y - h) * x * y) / (w * h);
    }
    ...
    public float tileableNoise3(float x,
            ...
            float d)
    {
        return (this.noise3(x, y, z) * (w - x) * (h - y) * (d - z) +
                this.noise3(x - w, y, z) * x * (h - y) * (d - z) +
                this.noise3(x, y - h, z) * (w - x) * y * (d - z) +
                this.noise3(x - w, y - h, z) * x * y * (d - z) +
                this.noise3(x, y, z - d) * (w - x) * (h - y) * z +
                this.noise3(x - w, y, z - d) * x * (h - y) * z +
                this.noise3(x, y - h, z - d) * (w - x) * y * z +
                this.noise3(x - w, y - h, z - d) * x * y * z) /
                (w * h * d);
    }
    ...
    {
        this.p = new int[this.B + this.B + 2];
        this.g3 = new float[this.B + this.B + 2][3];
        this.g2 = new float[this.B + this.B + 2][2];
        this.g1 = new float[this.B + this.B + 2];
        int i, j, k;
        ...
        for (i = 0; i < this.B; i++)
        {
            ...
            this.g1[i] = (float) (((this.rand.nextDouble() * Integer.MAX_VALUE) % (this.B + this.B)) - this.B) / this.B;

            for (j = 0; j < 2; j++)
            {
                this.g2[i][j] = (float) (((this.rand.nextDouble() * Integer.MAX_VALUE) % (this.B + this.B)) - this.B) / this.B;
            }
            this.normalize2(this.g2[i]);

            for (j = 0; j < 3; j++)
            {
                this.g3[i][j] = (float) (((this.rand.nextDouble() * Integer.MAX_VALUE) % (this.B + this.B)) - this.B) / this.B;
            }
        ...
        {
            k = this.p[i];
            j = (int) ((this.rand.nextDouble() * Integer.MAX_VALUE) % this.B);
            this.p[i] = this.p[j];
            this.p[j] = k;
        }
