...
/**
 * This is a Voronoi noise generator, originally from https://github.com/TJHJava/libnoiseforjava It was modified to work in a similar way to the bukkit noise generators, and to
 * support octaves and 2d noise,
 *
    ...
    public float noise(double x, double z, double frequency)
    {
        ...
        for (int zCur = zInt - 2; zCur <= zInt + 2; zCur++)
        {
            for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++)
            {
                ...
                double xPos = xCur + valueNoise2D(xCur, zCur, this.seed);
                double zPos = zCur + valueNoise2D(xCur, zCur, new Random(this.seed).nextLong());
                double xDist = xPos - x;
                double zDist = zPos - z;
    ...
    public float border(double x, double z, double width, float depth)
    {
        ...
        for (int zCur = zInt - 2; zCur <= zInt + 2; zCur++)
        {
            for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++)
            {
                ...
                double zDist = zPos - z;
                // double dist = xDist * xDist + zDist * zDist;
                double dist = this.getDistance2D(xPos - x, zPos - z);
        ...
        for (int zCur = zInt - 2; zCur <= zInt + 2; zCur++)
        {
            for (int yCur = yInt - 2; yCur <= yInt + 2; yCur++)
            {
                for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++)
                {
                    ...
                    double xPos = xCur + valueNoise3D(xCur, yCur, zCur, this.seed);
