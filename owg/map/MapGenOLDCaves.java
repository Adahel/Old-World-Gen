...
public class MapGenOLDCaves extends MapGenOLD
{
    ...
    protected void func_870_a(int i, int j, ChunkPrimer chunk, double d, double d1,
            double d2)
    {
        this.releaseEntitySkin(i, j, chunk, d, d1, d2, 1.0F + this.rand.nextFloat() * 6F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void releaseEntitySkin(int i, int j, ChunkPrimer chunk, double d, double d1,
            ...
            double d3)
    {
        ...
        for (; k < l; k++)
        {
            ...
            if (!flag && k == j1 && f > 1.0F)
            {
                this.releaseEntitySkin(i, j, chunk, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - ((float) Math.PI / 2F), f2 / 3F, k, l, 1.0D);
                this.releaseEntitySkin(i, j, chunk, d, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + ((float) Math.PI / 2F), f2 / 3F, k, l, 1.0D);
                return;
            }
            ...
            for (int j2 = d8; !flag2 && j2 < k1; j2++)
            {
                for (int l2 = d10; !flag2 && l2 < i2; l2++)
                {
                    ...
                    {
                        int j3 = j2 << 12 | l2 << 8 | i3;
                        if (i3 < 0 || i3 >= 128)
                        {
                            continue;
                        }
                        Block block = chunk.getBlockState(j3).getBlock();
            ...
            for (int k2 = d8; k2 < k1; k2++)
            {
                ...
                label0: for (int k3 = d10; k3 < i2; k3++)
                {
                    ...
                    do
                    {
                        ...
                        {
                            Block byte0 = chunk.getBlockState(l3).getBlock();
                            ...
                            if (byte0 == Blocks.stone || byte0 == Blocks.dirt || byte0 == Blocks.grass)
                            {
                                if (i4 < 10)
                                {
                                    chunk.setBlockState(l3, Blocks.lava.getDefaultState());
                                }
                                else
                                {
                                    chunk.setBlockState(l3, Blocks.air.getDefaultState());
                                    if (flag3 && chunk.getBlockState(l3 - 1).getBlock() == Blocks.dirt)
                                    {
                                        chunk.setBlockState(l3 - 1, Blocks.grass.getDefaultState());
                                    }
                                }
                            }
                        }
    ...
    protected void recursiveGenerate(World world, int i, int j, int k, int l, ChunkPrimer chunk)
    {
        ...
        for (int j1 = 0; j1 < i1; j1++)
        {
            ...
            {
                this.func_870_a(k, l, chunk, d, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; l1++)
            {
                ...
                this.releaseEntitySkin(k, l, chunk, d, d1, d2, f2, f, f1, 0, 0, 1.0D);
            }
