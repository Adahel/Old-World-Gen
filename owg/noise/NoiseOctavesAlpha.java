...
public class NoiseOctavesAlpha extends OldNoiseGenerator2
{
    ...
    {
        this.field_1191_b = i;
        this.field_1192_a = new NoisePerlinAlpha[i];
        for (int j = 0; j < i; j++)
        {
            this.field_1192_a[j] = new NoisePerlinAlpha(random);
        }
    ...
    public double[] func_4109_a(double ad[], int i, int j, int k, int l, double d,
            double d1, double d2)
    {
        return this.func_807_a(ad, i, 10D, j, k, 1, l, d, 1.0D, d1);
    }

    private NoisePerlinAlpha field_1192_a[];
