...
public class ChunkGeneratorBeta implements IChunkProvider
{
    ...
    public ChunkGeneratorBeta(World world, long l, boolean par4, int bSettings, int bRavine)
    {
        ...
        this.chunkManager = new ManagerOWG(world, !world.isRemote, bSettings);
        ...
        if (this.biomeSettings > 0)
        {
            ...
            for (int k = 0; k < 16; k++)
            {
                for (l = 0; l < 16; l++)
                {
                    ...
                    biomegenbase.handle.genTerrainBlocks(this.worldObj, this.rand, chunk, i * 16 + k, j * 16 + l, this.stoneNoise[l + k * 16]);
                }
            }
        }
        else
    ...
    public Chunk provideChunk(int i, int j)
    {
        ...
        if (this.biomeSettings == 0)
        {
            ...
            {
                abyte1[k] = (byte) this.biomesForGeneration[k].handle.biomeID;
            }
        }
        else
        {
            for (int k = 0; k < abyte1.length; k++)
            {
                abyte1[k] = (byte) this.biomesForGeneration[(int) (((k * 16) & 0xff) + (int) (k / 16))].handle.biomeID;
            }
        }
    ...
    public void populate(IChunkProvider ichunkprovider, int i, int j)
    {
        if (this.biomeSettings == 0)
        {
            ...
            if (biomegenbase == BiomeGenBase.OLDforest)
            {
                l7 += k4 + 5;
            }
            if (biomegenbase == BiomeGenBase.OLDrainforest)
            {
                l7 += k4 + 5;
            }
            if (biomegenbase == BiomeGenBase.OLDseasonalForest)
            {
                l7 += k4 + 2;
            }
            if (biomegenbase == BiomeGenBase.OLDtaiga)
            {
                l7 += k4 + 5;
            }
            if (biomegenbase == BiomeGenBase.OLDdesert)
            {
                l7 -= 20;
            }
            if (biomegenbase == BiomeGenBase.OLDtundra)
            {
                l7 -= 20;
            }
            if (biomegenbase == BiomeGenBase.OLDplains)
            {
                l7 -= 20;
            }
            ...
            for (int l14 = 0; l14 < byte1; l14++) // GOOD
            {
                ...
                if (biomegenbase == BiomeGenBase.OLDrainforest && this.rand.nextInt(3) != 0)
                {
            ...
            SpawnerAnimals.performWorldGenSpawning(this.worldObj, this.chunkManager.getBiomeGenAtChunkCoord(chunkcoordintpair).handle, k + 8, l + 8, 16, 16,
                    this.rand2);

            MinecraftForge.EVENT_BUS.post(new PopulateChunkEvent.Pre(ichunkprovider, this.worldObj, this.rand2, i, j, false));

            this.generatedTemperatures = this.chunkManager.getColdTemperatures(this.generatedTemperatures, k + 8, l + 8, 16, 16);
        ...
        else
        {
            ...
            biomegenbase.handle.decorate(this.worldObj, this.rand, new BlockPos(k, 0, l));
    ...
    public void recreateStructures(Chunk chunk, int par1, int par2)
    {
        if (this.mapFeaturesEnabled)
        {
            this.strongholdGenerator.generate(this, this.worldObj, par1, par2, (ChunkPrimerBeta) null);
            this.mineshaftGenerator.generate(this, this.worldObj, par1, par2, (ChunkPrimerBeta) null);
        }
    }
}
