...
public class ChunkGeneratorIndev implements IChunkProvider
{
    ...
    public ChunkGeneratorIndev(World par1World, long par2, boolean par4, int type, int theme, int s, int l, int d, int bSettings)
    {
        ...
        this.dungeons = d;
        if (this.biomeSettings == 4)
        {
            this.chunkManager = new ManagerOWGHell(BiomeGenBase.CLASSICsnow, 0.0D, 0.5D);
        }
        else
        {
            this.chunkManager = new ManagerOWGHell(BiomeGenBase.CLASSICnormal, 1.0D, 0.0D);
        }
    ...
    public void generateSkylands(int par1, int par2, ChunkPrimer chunk)
    {
    ...
    public void generateSurface(int par1, int par2, ChunkPrimer chunk)
    {
        int jj = 0;
    ...
    public void generateTerrain(int par1, int par2, ChunkPrimer chunk)
    {
        if (this.worldObj.getWorldInfo().getSpawnX() != 0)
        {
            this.worldObj.getWorldInfo().setSpawn(new BlockPos(0, 256, 0));
    ...
    public Chunk provideChunk(int cx, int cy)
    {
        ...
        {
            var5[var6] = (byte) this.biomesForGeneration[var6].handle.biomeID;
        }
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
        ...
        if (par2 == (int) Math.floor(this.worldObj.getWorldInfo().getSpawnX() / 16) && par3 == (int) Math.floor(this.worldObj.getWorldInfo().getSpawnZ() / 16))
        {
        ...
        if (this.typeFloating)
        {
            if (par2 == 0 && par3 == 0 && this.dungeons < 2)
            {
        ...
        SpawnerAnimals.performWorldGenSpawning(this.worldObj, this.chunkManager.getBiomeGenAtChunkCoord(chunkcoordintpair).handle, var4 + 8, var5 + 8, 16, 16,
                this.rand);
