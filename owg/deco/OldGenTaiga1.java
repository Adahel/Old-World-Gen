    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        ...
        for (int l1 = j; l1 <= j + 1 + l && flag; l1++)
        {
            ...
            for (int l2 = i - j2; l2 <= i + j2 && flag; l2++)
            {
                for (int k3 = k - j2; k3 <= k + j2 && flag; k3++)
                {
                    if (l1 >= 0 && l1 < 128)
                    {
                        Block j4 = OWGGenHelper.getBlock(world, l2, l1, k3);
        ...
        for (int i3 = j + l; i3 >= j + i1; i3--)
        {
            for (int l3 = i - k2; l3 <= i + k2; l3++)
            {
                ...
                {
                    int i5 = l4 - k;
                    if ((Math.abs(k4) != k2 || Math.abs(i5) != k2 || k2 <= 0) && !OWGGenHelper.getBlock(world, l3, i3, l4).isOpaqueCube())
                    {
                        OWGGenHelper.setBlock(world, l3, i3, l4, Blocks.leaves, BlockPlanks.EnumType.SPRUCE);
                    }
                }
        ...
        {
            Block i4 = OWGGenHelper.getBlock(world, i, j + j3, k);
            if (i4 == Blocks.air || i4 == Blocks.leaves)
            {
                OWGGenHelper.setBlock(world, i, j + j3, k, Blocks.log, BlockPlanks.EnumType.SPRUCE);
            }
        }
